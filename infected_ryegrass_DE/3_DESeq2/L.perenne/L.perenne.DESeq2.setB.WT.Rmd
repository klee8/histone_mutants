...
setB_list <- as.vector(samples[samples$condition == "setB", "run"])
...
setB_samplenum <- length(as.vector(unique(samples[samples$condition == "setB", "sample"])))
WT_samplenum <- length(as.vector(unique(samples[samples$condition == "WT", "sample"])))

salmon_TPM <- as.data.frame (txi$abundance) %>% 
  mutate( mean_setB_TPM = rowSums(select(., setB_list)/setB_samplenum),
          mean_WT_TPM = rowSums(select(., WT_list)/WT_samplenum),
...
pdf("plots/setB_vs_WT_top_DE_genes.pdf" )
...
write.table(goi, "result_tables/L.perenne_setB_WT.txt", quote = FALSE, row.names = FALSE, sep = "\t")
```
