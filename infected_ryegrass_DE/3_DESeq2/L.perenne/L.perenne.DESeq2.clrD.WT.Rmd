...
samples <- samples[samples$condition == "WT" | samples$condition == "clrD",]
```
...
clrD_list <- as.vector(samples[samples$condition == "clrD", "run"])
...
pdf("plots/clrD_vs_WT_rlog2_normalised_log2FC.pdf", onefile = FALSE)
...
pdf("plots/clrD_vs_WT_heat_map_sample_dist_rld-normalised_data.pdf", onefile = FALSE)
...
pdf("plots/clrD_v_WT_PCA_plot_rld-normalised_data.pdf", onefile = FALSE)
...
pdf("plots/clrD_vs_WT_top_DE_genes.pdf" )
...
```{r plot_dispersions}
pdf("plots/clrD_vs_WT_plot_dispersions.pdf", onefile = FALSE)
plotDispEsts(dds)
dev.off()
```
...
resApeT_thresh1  <- lfcShrink(dds, coef="condition_clrD_vs_WT", type="apeglm", lfcThreshold = 1)
resApeT_thresh2  <- lfcShrink(dds, coef="condition_clrD_vs_WT", type="apeglm", lfcThreshold = 2)
...
pdf("plots/clrD_vs_WT_volcano_plot.pdf", onefile = FALSE)
...
goi <- all_results %>% select(gene_id, apeglm_log2FC, apeglm_lfcSE, apeglm_1_svalue, apeglm_2_svalue, mean_WT_TPM, mean_clrD_TPM)



write.table(goi, "result_tables/L.perenne_clrD_WT.txt", quote = FALSE, row.names = FALSE, sep = "\t")
```
