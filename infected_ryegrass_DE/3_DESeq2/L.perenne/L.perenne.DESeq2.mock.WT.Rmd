...
samples <- samples[samples$pop == "L.perenne",]

# subset conditions of interest
samples <- samples[samples$condition == "mock" | samples$condition == "WT",]
```
...
tx2gene <- read.table(file.path(dir, "transcriptome/L.perenne/EST.headers.txt"))
...
WT_list <- as.vector(samples[samples$condition == "WT", "run"])
mock_list <- as.vector(samples[samples$condition == "mock", "run"])

# divide by the number of unique samples (some samples are duplicated over multiple lanes)
WT_samplenum <- length(as.vector(unique(samples[samples$condition == "WT", "sample"])))
mock_samplenum <- length(as.vector(unique(samples[samples$condition == "mock", "sample"])))

salmon_TPM <- as.data.frame (txi$abundance) %>% 
  mutate( mean_WT_TPM = rowSums(select(., WT_list)/WT_samplenum),
          mean_mock_TPM = rowSums(select(., mock_list)/mock_samplenum),
          gene_id = rownames(txi$abundance)) %>%
  select(gene_id, mean_WT_TPM, mean_mock_TPM) 
```
...
pdf("plots/WT_vs_mock_rlog2_normalised_log2FC.pdf", onefile = FALSE)
...
pdf("plots/WT_vs_mock_heat_map_sample_dist_rld-normalised_data.pdf", onefile = FALSE)
...
pdf("plots/WT_v_mock_PCA_plot_rld-normalised_data.pdf", onefile = FALSE)
...
pdf("plots/mock_vs_WT_top_DE_genes.pdf" )
...
```{r plot_dispersions}
pdf("plots/mock_vs_WT_plot_dispersions.pdf", onefile = FALSE)
plotDispEsts(dds)
dev.off()
```
...
resApeT_thresh1  <- lfcShrink(dds, coef="condition_WT_vs_mock", type="apeglm", lfcThreshold = 1)
resApeT_thresh2  <- lfcShrink(dds, coef="condition_WT_vs_mock", type="apeglm", lfcThreshold = 2)


pdf("plots/mock_vs_WT_apeglm_shrinkage_MA_plot.pdf", onefile = FALSE)
...
pdf("plots/mock_vs_WT_volcano_plot.pdf", onefile = FALSE)
...
goi <- all_results %>% select(gene_id, apeglm_log2FC, apeglm_lfcSE, apeglm_1_svalue, apeglm_2_svalue, mean_mock_TPM, mean_WT_TPM)



write.table(goi, "result_tables/L.perenne_mock_WT.txt", quote = FALSE, row.names = FALSE, sep = "\t")
```
...
# add EST name
colnames(tx2gene) <- c( "EST_id","gene_id")
goi <- merge(goi, tx2gene, by= "gene_id")

# highlight significant fold changes
goi <- goi %>% mutate(FC_2_up = ifelse(apeglm_log2FC >= 1 & apeglm_1_svalue < 0.005 & !is.na(apeglm_log2FC), 1, 0),
               FC_4_up = ifelse(apeglm_log2FC >= 2 & apeglm_2_svalue < 0.005 & !is.na(apeglm_log2FC), 1, 0),
               FC_2_down = ifelse(apeglm_log2FC <= -1 & apeglm_1_svalue < 0.005 & !is.na(apeglm_log2FC), 1, 0),
               FC_4_down = ifelse(apeglm_log2FC <= -2 & apeglm_2_svalue < 0.005 & !is.na(apeglm_log2FC),1, 0))

# add pierre's automated annotations
ann <- read.delim("../../Pierres_annotations/nph13614-sup-0002-tables1.csv", header = TRUE, sep = "\t", stringsAsFactors = FALSE, skip = 2)
goi <- merge(goi, ann, by.x = "EST_id", by.y = "Identifier")
goi$X <- NULL
goi$X.1 <- NULL

head(goi)

write.table(goi, "result_tables/L.perenne_mock_vs_WT_annotated.txt", quote = FALSE, row.names = FALSE, sep = "\t")
```
