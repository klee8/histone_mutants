---
title: "E.festucae_Fl1_setb_vs_wt"
...
#if (!requireNamespace("BiocManager", quietly = TRUE))
#    install.packages("BiocManager")
#BiocManager::install("tximportData", version = "3.9")
...
run = illumina base-name for the fq file, or something similar which indicates sample, lane etc (must be unique for each row)
...
samples <- samples[samples$condition == "WT" | samples$condition == "setB",]
```
...
salmon_TPM <- as.data.frame (txi$abundance) %>% 
  mutate( mean_setB_TPM = ((S10_1 + S10_2 + S11_1 + S11_2 + S12_1 + S12_2)/3),
          mean_wt_TPM = ((W4_1 + W4_2 + W5_1 + W5_2 + W6_1 + W6_2)/3),
          gene_id = rownames(txi$abundance)) %>%
  select(gene_id, mean_setB_TPM, mean_wt_TPM)

# tried for a general solution:
#temp1 <- as.vector(samples[samples$condition == "clrD", "run"])
#salmon_TPM <- as.data.frame (txi$abundance) %>% select(is %in% as.vector(samples[samples$condition == "clrD", "run"])) %>% colnames
...
pdf("plots/setB_vs_WT_rlog2_normalised_log2FC.pdf", onefile = FALSE)
...
pdf("plots/setB_vs_wt_heat_map_sample_dist_rld-normalised_data.pdf", onefile = FALSE)
...
pdf("plots/setB_v_wt_PCA_plot_rld-normalised_data.pdf", onefile = FALSE)
...
pdf("plots/setB_vs_wt_top_DE_genes.pdf", onefile = FALSE)
...
```{r plot_dispersions}
pdf("plots/setB_vs_wt_plot_dispersions.pdf", onefile = FALSE)
plotDispEsts(dds)
dev.off()
```
...
resApeT_thresh1  <- lfcShrink(dds, coef="condition_setB_vs_WT", type="apeglm", lfcThreshold = 1)
resApeT_thresh2  <- lfcShrink(dds, coef="condition_setB_vs_WT", type="apeglm", lfcThreshold = 2)

pdf("plots/setB_vs_wt_apeglm_shrinkage_MA_plot.pdf", onefile = FALSE)
...
pdf("plots/setB_vs_wt_volcano_plot.pdf", onefile = FALSE)
...
# merge in normalised count data 
#head(norm_counts)
# group colnames by config file 'condition' and get average.
#conditions <- as.vector(unique(samples$condition))
#ean_counts <- sapply(conditions, function(x) as.integer(rowMeans(normalized_counts[,colnames(normalized_counts) %in% 
...
goi <- all_results %>% select(gene_id, apeglm_log2FC, apeglm_lfcSE, apeglm_1_svalue, apeglm_2_svalue, mean_wt_TPM, mean_setB_TPM)
...
write.table(goi, "result_tables/E.festucae_Fl1_WT_setB.txt", quote = FALSE, row.names = FALSE, sep = "\t")
...
3556.4429*0.9056802      # normalised count X normalisation factor = raw count
```
