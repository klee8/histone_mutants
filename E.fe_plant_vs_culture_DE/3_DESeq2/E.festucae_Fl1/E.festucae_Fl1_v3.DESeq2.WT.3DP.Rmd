---
title: "E.festucae_Fl1_v3_WT_3DP_vs_WT_CULT"
...
samples <- read.table("../../config_2culture_samples.txt", header = FALSE)
...
samples <- samples[samples$condition == "WT_3DP" | samples$condition == "WT_CULT",]
samples
```
...
dir <- "../../2_salmon"
files <- file.path(dir, "salmon_quant", "E.festucae_Fl1", samples$run, "quant.sf")
...
salmon_TPM <- as.data.frame (txi$abundance) %>% 
  mutate( mean_WT_3DP_TPM = ((W4_1 + W4_2 + W5_1 + W5_2 + W6_1 + W6_2)/3),
          #mean_WT_CULT_TPM = ((SRR7640306 +SRR7640308 + SRR7640309)/3),      # removed potentially contaminated culture sample SRR7640306
          mean_WT_CULT_TPM = ((SRR7640308 + SRR7640309)/3),
          gene_id = rownames(txi$abundance)) %>%
  select(gene_id, mean_WT_3DP_TPM, mean_WT_CULT_TPM)
...
pdf("plots/WT_3DP_vs_WT_CULT_rlog2_normalised_log2FC.pdf", onefile = FALSE)
...
pdf("plots/WT_3DP_vs_WT_CULT_heat_map_sample_dist_rld-normalised_data.pdf", onefile = FALSE)
...
pdf("plots/WT_3DP_vs_WT_CULT_PCA_plot_rld-normalised_data.pdf", onefile = FALSE)
...
pdf("plots/WT_3DP_vs_WT_CULT_top_DE_genes.pdf" )
...
```{r plot_dispersions}
pdf("plots/WT_3DP_vs_WT_CULT_plot_dispersions.pdf", onefile = FALSE)
plotDispEsts(dds)
dev.off()
```
...
resApeT_thresh1  <- lfcShrink(dds, coef="condition_WT_3DP_vs_WT_CULT", type="apeglm", lfcThreshold = 1)
resApeT_thresh2  <- lfcShrink(dds, coef="condition_WT_3DP_vs_WT_CULT", type="apeglm", lfcThreshold = 2)
resultsNames(dds)

pdf("plots/WT_3DP_vs_WT_CULT_apeglm_shrinkage_MA_plot.pdf", onefile = FALSE)
...
pdf("plots/WT_3DP_vs_WT_CULT_volcano_plot.pdf", onefile = FALSE)
...
head(all_results)
goi <- all_results %>% select(gene_id, apeglm_1_log2FC, apeglm_1_lfcSE, apeglm_1_svalue, apeglm_2_svalue, mean_WT_3DP_TPM, mean_WT_CULT_TPM)
colnames(goi) <- c("gene_id", "3DPvCULT_apeglm_log2FC", "3DPvCULT_apeglm_lfcSE", "3DPvCULT_apeglm_1_svalue", "3DPvCULT_apeglm_2_svalue", "mean_WT_3DP_TPM", "mean_WT_CULT_TPM")
head(goi)




write.table(goi, "result_tables/E.festucae_Fl1_v3_WT_3DP_vs_WT_CULT.txt", quote = FALSE, row.names = FALSE, sep = "\t")
```
